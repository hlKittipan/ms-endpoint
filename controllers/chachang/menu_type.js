...
const menuType = require("../../models/chachang/menu_types");
...
module.exports = {
  ...
  fetchData: async (req, res, next) => {
    try {
      const result = await menuType.findAvailable();
  ...
  store: async (req, res, next) => {
    ...
    try {
      const data = new menuType({...req.body,});
  ...
  update: async (req, res, next) => {
    ...
    try {
      const item = req.body
      const result = await menuType.findOneAndUpdate({_id: req.params.id,},{...item},{ new: true });
      if (result) {
        res.status(200).send(result);
      }else{
  ...
  delete: async (req, res, next) => {
    ...
    try {
      const result = await menuType.softDelete(req.params.id);
      if (result) {
