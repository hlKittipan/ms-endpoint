...
const AddOnType = require("../../models/pos/add_on_type");
...
module.exports = {
  ...
  fetchData: async (req, res, next) => {
    try {
      const result = await AddOnType.findAvailable();
  ...
  store: async (req, res, next) => {
    ...
    try {
      const data = new AddOnType({...req.body,});
  ...
  update: async (req, res, next) => {
    ...
    try {
      const item = req.body
      const result = await AddOnType.findOneAndUpdate({_id: req.params.id,},{...item},{ new: true });
      if (result) {
        res.status(200).send(result);
      }else{
  ...
  delete: async (req, res, next) => {
    ...
    try {
      const result = await AddOnType.softDelete(req.params.id);
      if (result) {
